...
var nativeSelect = function nativeSelect(_ref) {
  ...
  }), '\n    ']), _select), '\n    ', (_ = document.createElement('div'), _.setAttribute('class', '' + String(classnames('m-dropdown__select-icon', _defineProperty({}, 'm-dropdown__select-icon--' + size, size))) + ''), _appendChild(_, [raw(arrowIcon)]), _), '\n  ']), _div;
};
...
var enhancedSelect = function enhancedSelect(_ref3) {
  ...
  var title = _ref3.title,
      items = _ref3.items,
      size = _ref3.size;
  return [(_button = document.createElement('button'), _button.setAttribute('type', 'button'), _button.setAttribute('class', '' + String(classnames('m-dropdown__toggle js-dropdown__toggle', _defineProperty({}, 'm-dropdown__toggle--' + size, size))) + ''), _appendChild(_button, ['\n    ', title, raw(arrowIcon), '\n  ']), _button), (_mDropdownContent = document.createElement('ul'), _mDropdownContent.setAttribute('class', 'm-dropdown__content'), _appendChild(_mDropdownContent, ['\n    ', Array.isArray(items) && items.map(function (_ref4) {
...
export default (function (_ref5) {
  var native = _ref5.native,
      props = _objectWithoutProperties(_ref5, ['native']);
