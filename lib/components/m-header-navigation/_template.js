...
var submenuItem = function submenuItem(_ref) {
  ...
  })) + ''), _appendChild(_a, [raw(name)]), _a), '\n      ', Array.isArray(items) ? (_axaHeaderSubNavigation = document.createElement('axa-header-sub-navigation'), _axaHeaderSubNavigation.setAttribute('index-title', '' + String(name) + ''), _axaHeaderSubNavigation.setAttribute('index-url', '' + String(url) + ''), _axaHeaderSubNavigation.setAttribute('items', '' + String(_JSON$stringify(items)) + ''), _axaHeaderSubNavigation.setAttribute('flyout', 'flyout'), _appendChild(_axaHeaderSubNavigation, ['\n        ']), _axaHeaderSubNavigation) : '', '\n    ']), _mHeaderNavigationListItem;
};
...
export default (function (_ref2) {
  ...
  return _mHeaderNavigationList = document.createElement('ul'), _mHeaderNavigationList.setAttribute('class', 'm-header-navigation__list js-header-navigation__list'), _appendChild(_mHeaderNavigationList, ['\n    ', Array.isArray(items) && items.map(function (item) {
    return submenuItem(_extends({}, item, { simplemenu: simplemenu }));
