...
export function toggle(node) {
  ...
  if (Array.isArray(hasResult)) {
    ...
    var _hasResult$reduce2 = babelHelpers.slicedToArray(_hasResult$reduce, 2);
  ...
  if (hasResult !== false) {
    remove.apply(undefined, [node].concat(babelHelpers.toConsumableArray(classesToRemove)));
  }

  if (hasResult !== true) {
    add.apply(undefined, [node].concat(babelHelpers.toConsumableArray(classesToAdd)));
  }
}
