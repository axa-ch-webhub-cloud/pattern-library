  ...
  return html`
    <li class="m-header-mobile-navigation__list-item">
      ${hasItems
        ? [
            html`
              <button type="button" class="${categoryClass}">
                ${name}
                <axa-icon
                  icon="angle-bracket-down"
                  icon-class="a-icon__svg--small"
                  classes="m-header-mobile-navigation__icon-next"
                ></axa-icon>
              </button>
            `,
        ...
        : html`
            <a class="${linkListClass}" href="${url}">${name}</a>
          `}
    </li>
  `;
...
function mobileNav(items, parent) {
  return html`
    <div
      class="${classnames('m-header-mobile-navigation__nav', {
    ...
    >
      ...
      <ul class="m-header-mobile-navigation__list">
        ${parent && mobileNavItem({ ...parent, name: 'index page', items: null })} ${Array.isArray(items) && items.map(mobileNavItem)}
      </ul>
    </div>
  `;
}
