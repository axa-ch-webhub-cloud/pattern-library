...
const arrowIcon =
  '<axa-icon size="sm" icon="angle-bracket-down" classes="m-dropdown__icon"></axa-icon>';

export default ({ native, title, items = [], size }) => html`
<article class="m-dropdown__wrap">
      <div class="${classnames(
        "m-dropdown__select-wrap",
        { [`m-dropdown__select-wrap--${size}`]: size },
        "m-dropdown__list--native"
      )} ${native ? " m-dropdown__list--native--only" : ""}">
        <select class="${classnames(
          "m-dropdown__select",
          "js-dropdown__native-select",
      ...
      <div class="m-dropdown__list--enhanced" ${native ? "hidden" : ""}>
        <button type="button" class="${classnames(
          "m-dropdown__toggle js-dropdown__toggle",
        ...
        <ul class="m-dropdown__content js-dropdown__content">
          ${items &&
            items.map(
              ({ name, url = "#", isSelected, value }, index) => html`
                <li class="m-dropdown__item">
                  <a
                    tabindex="-1"
                    class="m-dropdown__link js-dropdown__link"
                    data-name="${name}"
                    data-index="${index}"
                    data-value="${value}"
