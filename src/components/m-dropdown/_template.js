...
const nativeSelect = ({ title, items, size }) => [html`
...
const enhancedSelect = ({ title, items, size }) => [html`
  ...
  html`<ul class="m-dropdown__content js-dropdown__content">
    ${Array.isArray(items) && items.map(({ name, url, isSelected, value }, index) => {
    return html`
      <li class="m-dropdown__item">
        <a class="m-dropdown__link" data-index="${index}" data-value="${value}" data-selected="${isSelected ? 'true' : 'false'}" href="${url}">${name}</a>
      </li>
    `;
  })}
  </ul>`,
