...
fixture('Button - basic functionality').page(`${host}/iframe.html?id=examples-button-pure-html--clickable`);
...
test('should inherit width to internal button', async t => {
  ...
  await t.expect($axaButtonShadowEl.getStyleProperty('width')).eql(expectedWidth);
});

fixture('Button - set properties').page(`${host}/iframe.html?id=components-button--button`);
...
fixture('Button - render icon').page(`${host}/iframe.html?id=examples-button-pure-html--icon-visible`);
...
fixture('Button - react smoke test').page(`${host}/iframe.html?id=examples-button-react--story`);
...
fixture('Button - Works in a form').page(`${host}/iframe.html?id=examples-button-pure-html--in-a-form`);

test('should submit only once', async t => {
  const $submitButton = await Selector('.js-submit-prevent');
  await t.click($submitButton());
  await t.wait(100);
  const count = ClientFunction(() => document.querySelector('.js-submit-prevent').dataset.count);
