...
fixture('Input File - basic functionality').page(`${host}/iframe.html?id=components-input-file--input-file`);
...
test('should pass width to internal button', async t => {
  ...
  await t.expect(axaInputFileLabel.getStyleProperty('width')).eql(expectedWidth);
});
...
fixture('Input File - set property accept').page(`${host}/iframe.html?id=components-input-file--input-file&knob-text=Upload&knob-accept=application/pdf`);
...
test('should set input file accept', async t => {
  ...
  await t.expect((await $axaElmInput.getAttribute('accept')) === 'application/pdf').ok();
});

fixture('Input File - set property multiple').page(`${host}/iframe.html?id=components-input-file--input-file&knob-text=Upload&knob-multiple=true`);
...
fixture('Input File - set property capture').page(`${host}/iframe.html?id=components-input-file--input-file&knob-text=Upload&knob-capture=true`);
...
fixture('Input File - react smoke test').page(`${host}/iframe.html?id=examples-input-file-react--story`);
...
test('should render react input file', async t => {
  ...
  const $axaButtonShadow = await Selector(() => document.querySelector('axa-input-file'));
