...
fixture('Input text - basic functionality').page(`${host}/iframe.html?id=components-input-text--input-text`);

test('should render input-text', async t => {
  const $axaElem = await Selector(TAG);
  await t.expect($axaElem.exists).ok();
  const $axaTag = await Selector(() => document.querySelector('axa-input-text'));
...
test('should show error message and have the right color', async t => {
  ...
  await setInvalid();
  const $axaError = await Selector(() => document.querySelector('.a-input-text__error'));
...
fixture('Input text - Form').page(`${host}/iframe.html?id=examples-input-text-pure-html--in-a-form`);
...
fixture('Input text - Max Length').page(`${host}/iframe.html?id=components-input-text--input-text&knob-label*=&knob-name*=&knob-refid=&knob-placeholder=&knob-value=&knob-error=&knob-info=&knob-type=text&knob-maxlength=5&knob-counterMax=Character%20limit%20reached!&knob-checkmark=true`);
...
test('should correctly show character count with counter within text', async t => {
  ...
  await t.expect($counterInfo.getStyleProperty('color')).eql('rgb(201, 20, 50)');
});

fixture('Input text - no maxlength').page(`${host}/iframe.html?id=examples-input-text-pure-html--no-maxlength-set`);
...
fixture('Input text - no counter').page(`${host}/iframe.html?id=examples-input-text-pure-html--no-counter-set`);
...
fixture('Input text - maxLength works with autocomplete').page(`${host}/iframe.html?id=examples-input-text-react--story-simulate-autocomplete`);
...
test('should cut text when autocomplete sets value over maxLength', async t => {
  ...
  await t.wait(2000);
  const inputValue = await ClientFunction(() => document.querySelector('#fix-id-86452623').value);
  await t.wait(1000);
  // story adds 123456789 but here it should be cut to the limit
  await t.expect(await inputValue()).eql('1234');
});

fixture('Input text - Set attributes "pattern" and "numeric"').page(`${host}/iframe.html?id=components-input-text--input-text&knob-label*=&knob-name*=&knob-refId=&knob-placeholder=&knob-error=&knob-info=&knob-defaultValue=&knob-type=text&knob-pattern=[0-9]*&knob-inputmode=numeric&knob-refid=&knob-value=&knob-maxlength=50&knob-counter=Still%20##counter##%20characters%20left&knob-counterMax=Over%20character%20limit!`);
...
fixture('Input text - autofocus').page(`${host}/iframe.html?id=components-input-text--input-text&knob-label*=&knob-name*=&knob-refid=&knob-placeholder=&knob-value=&knob-error=&knob-info=&knob-type=text&knob-maxlength=50&knob-autofocus=true&knob-counter=Still ##counter## characters left&knob-counterMax=Over character limit!&knob-pattern=&knob-inputmode=`);
...
fixture('Input text - defaultValue for react').page(`${host}/iframe.html?id=examples-input-text-react--story&knob-label*=&knob-name*=&knob-refId=&knob-placeholder=&knob-error=&knob-info=&knob-defaultValue=qwertz&knob-type=text&knob-pattern=&knob-inputmode=`);
...
fixture('Input text - currency on controlled component').page(`${host}/iframe.html?id=examples-input-text-react--controlled-uncontrolled`);

test('should format value of controlled component', async t => {
  const $axaInputElement = await Selector(() => document.querySelector('#controlled_currency')).find(CLASS);
...
fixture('Input text - currency formatting on first render react').page(`${host}/iframe.html?id=examples-input-text-react--story&knob-label*=&knob-name*=&knob-refId=&knob-placeholder=&knob-error=&knob-info=&knob-currency=chf&knob-defaultValue=&knob-value=45&knob-type=text&knob-pattern=&knob-inputmode=&viewMode=story`);
...
fixture('Input-Text - React onKeyUp').page(`${host}/iframe.html?id=examples-input-text-react--using-onkeyup-event&viewMode=story`);

test('should fire onKeyUp callback on user input', async t => {
  const testoutput = await Selector(() => document.querySelector('#inputtext-react-testoutput')).addCustomDOMProperties({
    innerHTML: el => el.innerHTML,
  });

  const realInputField = await Selector(() => document.querySelector('axa-input-text').querySelector('input'));
