        ...
        <ul class="m-meta-navigation__list">
          ${left && left.map(({ url, name }, index) => bel`
            <li class="m-meta-navigation__list-item">
              <a href="${url}" class="m-meta-navigation__list-link${index === 0 ? ' is-active' : ''}">${raw(name)}</a>
            </li>
          `)}
        </ul>
      </nav>
              ...
              <div class="m-meta-navigation__lang-drop-down js-dropdown">
                ...
                ${right && right.map(({ url, name }, index) => bel`
                  <li class="m-meta-navigation__lang-list-item">
                    <a class="m-meta-navigation__lang-list-link${index === 0 ? ' is-active' : ''}" href="${url}">${raw(name)}</a>
                  </li>
                `)}
                </ul>
              </div>
