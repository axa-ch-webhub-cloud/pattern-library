...
fixture('Commercial Hero Banner - basic functionality').page(`${host}/iframe.html?id=components-commercial-hero-banner--commercial-hero-banner`);
...
test('should render commercial-hero-banner', async t => {
  ...
  const $axaElemShadow = await Selector(() => document.querySelector(TAG).shadowRoot, { dependencies: { TAG } }).find(ROOT_CLASS);
  await t.expect($axaElemShadow.exists).ok();
});

fixture('Commercial Hero Banner - Dark Mode').page(`${host}/iframe.html?id=components-commercial-hero-banner--commercial-hero-banner&knob-Variant=dark&knob-Image%20Source=https://d5cplpsrt2s33.cloudfront.net/m/24c1b33e4e8ceda1/WIDE_1440_560_X2-hero_kv_neu_kv_breit_web.jpg`);
...
test('should render in dark mode', async t => {
  ...
  await t.expect(container.hasClass('o-commercial-hero-banner__container--dark')).ok();

  await _assertDarkBackgroundGradient(t, container);
});

async function _assertDarkBackgroundGradient(t, container) {
  await t.expect(container.getStyleProperty('background-image')).eql('linear-gradient(270deg, rgba(0, 0, 0, 0) 30%, rgba(0, 0, 0, 0.8) 60%, rgb(0, 0, 0) 100%)');
}

fixture('Commercial Hero Banner - Content')
  .page(`${host}/iframe.html?id=components-commercial-hero-banner--commercial-hero-banner&knob-Variant=light&knob-Image%20Source=https://d5cplpsrt2s33.cloudfront.net/m/24c1b33e4e8ceda1/WIDE_1440_560_X2-hero_kv_neu_kv_breit_web.jpg`)
  .beforeEach(async t => {
...
test('should render in light mode', async t => {
  ...
  const container = $axaElemShadow.find('.o-commercial-hero-banner__container');
  await t.expect(container.visible).ok();
  await t.expect(container.hasClass('o-commercial-hero-banner__container--dark')).notOk();
  await _assertLightBackgroundGradient(t, container);
});

async function _assertLightBackgroundGradient(t, container) {
  await t.expect(container.getStyleProperty('background-image')).eql('linear-gradient(270deg, rgba(255, 255, 255, 0) 30%, rgba(255, 255, 255, 0.9) 60%, rgb(255, 255, 255) 100%)');
}
...
test('should use correct typography for mobile view', async t => {
  ...
  await t.expect(title.getStyleProperty('font-family')).eql('"Publico Headline", Georgia, serif');
  ...
  await t.expect(content.getStyleProperty('font-family')).eql('"Source Sans Pro", Arial, sans-serif');
  ...
  await t.expect(disclaimer.getStyleProperty('font-family')).eql('"Source Sans Pro", Arial, sans-serif');
...
fixture('Commercial Hero Banner - Check addons')
  .page(`${host}/iframe.html?id=examples-commercial-hero-banner-pure-html--with-badges`)
  .beforeEach(async t => {
    await t.maximizeWindow();
  });
...
test('should render badge and addon', async t => {
  ...
  const $addon = $axaHeroBanner.find('.o-commercial-hero-banner-demo__addon-example');
  await t.expect($addon.visible).ok();

  const $badge = $axaHeroBanner.find('.o-commercial-hero-banner-demo__badge-wrapper');
  await t.expect($badge.visible).ok();
  await t.expect($badge.getStyleProperty('background-color')).eql('rgb(201, 20, 50)');
});
...
test('should set correct width for button', async t => {
  ...
  const $axaButtonLinkShadow = await Selector(() => document.querySelector('axa-button-link').shadowRoot);

  const $axaButtonShadowElement = await $axaButtonLinkShadow.find('.a-button-link');
  ...
  await t.expect($axaButtonShadowElement.getStyleProperty('width')).eql(expectedWidth);
});
