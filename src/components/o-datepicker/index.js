...
class AXADatepicker extends BaseComponentGlobal {
  ...
  connectedCallback() {
    ...
    this.onDatepickerCalendarClick = on(
      this,
      EVENTS.CLICK,
      "js-datepicker__calendar",
      e => this.handleDatepickerCalendarClick(e)
    );
    this.onDatePickerInputClick = on(this, EVENTS.CLICK, "js-input__input", e =>
      this.handleDatepickerInputClick(e)
    );
    this.onDatePickerInputButtonClick = on(
      this,
      EVENTS.CLICK,
      "js-input__icon-button",
      e => this.handleDatepickerInputButtonClick(e)
    );
    ...
    this.onDatePickerInputChange = on(
      ...
      e => this.handleDatepickerInputChange(e),
      { capture: true, passive: false }
    );

    // Listen to fired events of sub component datepicker calendar
    this.onDatepickerCalendarDateChanged = on(
      this,
      "date-changed",
      "js-datepicker__calendar",
      e => this.handleDatepickerChangeDate(e)
    );
    this.onDatepickerCalendarCancel = on(
      this,
      "cancel",
      "js-datepicker__calendar",
      e => this.handleDatepickerCancel(e)
    );
    ...
    window.setTimeout(() => {
      ...
      window.addEventListener(
        "resize",
        debounce(() => this.handleViewportCheck(baseElemementInput), 250)
      );
      window.addEventListener(
        "scroll",
        debounce(() => this.handleViewportCheck(baseElemementInput), 250)
  ...
  didRenderCallback() {
    this.onInputFieldRender = on(
      this.querySelector(".js-datepicker__input"),
      AXA_EVENTS.AXA_RENDER,
      "",
      e => this.handleInputFieldRendered(e)
    );
  }
  ...
  closeDropdowns() {
    ...
    this.querySelector(".js-datepicker__dropdown-month").classList.remove(
      "is-dropdown-open"
    );
    this.querySelector(".js-datepicker__dropdown-year").classList.remove(
      "is-dropdown-open"
    );
  }
  ...
  isValidDate(date) {
    ...
    try {
      ...
      if (
        ...
        const isInValidationYearRange =
          this.allowedYears.indexOf(isValidDateLocalized.getFullYear()) > 0;
  ...
  handleDatepickerInputChange(e) {
    if (
      ...
      typeof e.detail.value === "undefined" ||
  ...
  updateDatepickerBody(date) {
    ...
    this.querySelector(".js-datepicker__datepicker-body").setAttribute(
      "date",
      date
    );
  }
  ...
  shouldMove(elem) {
    ...
    const moreSpaceOnTopThanBottom =
      elemement.top > window.innerHeight - elemement.bottom;
  ...
  disconnectedCallback() {
    ...
    this.body.removeEventListener(EVENTS.TOUCHEND, e =>
