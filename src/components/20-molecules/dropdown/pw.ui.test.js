...
describe('Dropdown', () => {
  it('should submit correct value to form', async () => {
    ...
    await page.goto(`${host}/iframe.html?id=examples-dropdown-pure-html--in-a-form`);
    await page.waitForSelector(dropdownSelector);

    await page.click(dropdownSelector);
    await page.click('axa-dropdown[data-test-id="dropdown-forms"] button[data-value="FR"]');
    await page.click('#dropdown-form axa-button[type="submit"]');

    expect(await page.textContent('#dropdown-form span[id="form-data-lang"]')).toBe('FR');
