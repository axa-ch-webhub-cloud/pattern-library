...
describe('Datepicker', () => {
  it('should submit datepicker correctly in form', async () => {
    ...
    await page.goto(`${host}/iframe.html?id=examples-datepicker-pure-html--in-a-form`);
    await page.waitForSelector(datepickerForm);

    await page.type(`axa-datepicker[data-test-id="datepicker-forms"] .js-datepicker__input`, '29.2.2020');
    await page.click('#datepicker-forms-submit');

    expect(await page.textContent('#form-data-date')).toBe('29.2.2020 (of 1 submittable elements)');
  });
  ...
  it('should not submit form if click on arrow buttons', async () => {
    ...
    expect(await page.textContent('#form-data-date')).not.toBe('29.02.2020 (of 1 submittable elements)');
  });
