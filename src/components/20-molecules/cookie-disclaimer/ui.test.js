...
fixture('Cookie disclaimer - basic functionality').page(`${host}/iframe.html?id=components-cookie-disclaimer--cookie-disclaimer`);
...
test('should store in localStorage the date of when accepted', async t => {
  const $axaElem = await Selector(TAG);
  await t.expect($axaElem.exists).ok();
  const $axaBtnShadow = await Selector(() => document.querySelector('axa-cookie-disclaimer').shadowRoot.querySelector('.js-button'));
  ...
  await t.expect(parseInt(await timeStamp(), 10)).within(time - 1000, time + 1000);
});
