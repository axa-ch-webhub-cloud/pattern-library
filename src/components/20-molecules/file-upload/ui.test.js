...
const validFiles = ['./ui-test-files/test.jpg', './ui-test-files/test.png', './ui-test-files/test.pdf', './ui-test-files/testbig.pdf'];

test('should render file-upload', async t => {
  const $axaElem = await Selector(FILE_UPLOAD_TAG);
  await t.expect($axaElem.exists).ok();
  const $axaElemShadow = await Selector(() => document.querySelector('axa-file-upload').shadowRoot);
...
test('should upload file and switch view', async t => {
  const $inputFileInputElem = await Selector(
    () => document.querySelector('axa-file-upload').shadowRoot.querySelector('.js-file-upload__input .a-input-file__input') // TODO change to .js-
  );
  await t.expect($inputFileInputElem.exists).ok();

  await t.setFilesToUpload($inputFileInputElem, validFiles);

  const $fileOverviewElem = await Selector(() => document.querySelector('axa-file-upload').shadowRoot.querySelector('.js-file-upload__dropzone-file-overview'));
...
test("shouldn't upload file because it's an invalid type", async t => {
  const $inputFileInputElem = await Selector(() => document.querySelector('axa-file-upload').shadowRoot.querySelector('.js-file-upload__input .a-input-file__input'));
  await t.expect($inputFileInputElem.exists).ok();
...
test('should convert .png file to .jpg', async t => {
  ...
  const $figcaptionElem = await Selector(() => document.querySelector('axa-file-upload').shadowRoot.querySelector('.js-file-upload__filename'));
...
test('should delete image', async t => {
  ...
  const $figureElemsBefore = await Selector(() => document.querySelector('axa-file-upload').shadowRoot.querySelectorAll('.js-file-upload__img-figure'));
  ...
  const $figureElem = await Selector(() => document.querySelector('axa-file-upload').shadowRoot.querySelector('.js-file-upload__img-figure'));

  await t.click($figureElem);

  const $figureElemsAfter = await Selector(() => document.querySelector('axa-file-upload').shadowRoot.querySelectorAll('.js-file-upload__img-figure'));
...
test('should exceed maximum size of single file', async t => {
  ...
  const $figureElems = await Selector(() => document.querySelector('axa-file-upload').shadowRoot.querySelectorAll('.js-file-upload__img-figure'));
  ...
  const $fileName = await Selector(() => document.querySelector('axa-file-upload').shadowRoot.querySelectorAll('.js-file-upload__error'));
...
test('should remove addMoreInputFile', async t => {
  ...
  const $addMoreInputFieldElem = await Selector(() => document.querySelector('axa-file-upload').shadowRoot.querySelectorAll('.js-file-upload__add-more'));
  await t.expect($addMoreInputFieldElem.exists).notOk();
});

test('should exceed maximum number of files', async t => {
  const $inputFileInputElem = await Selector(() => document.querySelector(FILE_UPLOAD_TAG).shadowRoot, { dependencies: { FILE_UPLOAD_TAG } }).find('.js-file-upload__input .a-input-file__input');

  await t.expect($inputFileInputElem.exists).ok();

  await t.setFilesToUpload($inputFileInputElem, validFiles);

  const $figureElems = await Selector(() => document.querySelector('axa-file-upload').shadowRoot).find('.js-file-upload__img-figure');

  await $figureElems(); // Travis failed sometimes a line below. (Bug #1335)
  await t.expect(await $figureElems.count).eql(1);

  const $errorWrapper = await Selector(() => document.querySelector('axa-file-upload').shadowRoot.querySelectorAll('.js-file-upload__error-wrapper'));

  await t.expect($errorWrapper.innerText).eql('You exceeded the maximum number of files');
});

fixture('File upload - maxSizeOfAllFilesKB prop').page(`${host}/iframe.html?id=components-file-upload--file-upload&knob-inputFileText=Upload%20file&knob-maxSizeOfSingleFileKB=100&knob-maxSizeOfAllFilesKB=1&knob-maxNumberOfFiles=10&knob-deleteStatusText=Delete&knob-addStatusText=Add%20more&knob-fileTooBigStatusText=File%20size%20exceeds%20maximum%20size&knob-filesTooBigStatusText=File%20sizes%20exceed%20maximum%20size&knob-tooManyFilesStatusText=You%20exceeded%20the%20maximum%20number%20of%20files&knob-orText=or&knob-infoText=Drag%20and%20drop%20to%20upload%20your%20file&knob-icon=cloud-upload&knob-headerText=The%20following%20files%20are%20being%20transferred:`);
...
fixture('File upload - Remove file event').page(`${host}/iframe.html?id=examples-file-upload-react--story&viewMode=story`);
test('should throw an event if a file was removed from the list', async t => {
  ...
  const $firstFile = await Selector(() => document.querySelector('axa-file-upload').querySelector('.m-file-upload__icon-hover-area'));
