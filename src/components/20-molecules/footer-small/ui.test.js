...
fixture('Footer Small - Static links').page(`${host}/iframe.html?id=components-footer-small--footer-small`);
...
fixture('Footer Small - Dynamic Links Demo').page(`${host}/iframe.html?id=examples-footer-small-pure-html--dynamic-links`);

test('should update indices on click', async t => {
  const germanLanguage = Selector(() => document.querySelector(`axa-footer-small`).shadowRoot.querySelector('a'));

  const lastClickedLanguageIndex = Selector('#active-language');
  await t.expect(lastClickedLanguageIndex.textContent).eql('Language - Index Clicked: -');
  await t.click(germanLanguage);
  await t.expect(lastClickedLanguageIndex.textContent).eql('Language - Index Clicked: 0');
});

fixture('Footer Small - React smoke test').page(`${host}/iframe.html?id=examples-footer-small-react--callbacks-on-language`);
...
fixture('Footer Small - React dynamic children test').page(`${host}/iframe.html?id=examples-footer-small-react--dynamic-change-of-children-upon-language-change`);

test('should update Reactified axa-footer-small when children change dynamically', async t => {
  const disclaimers = await Selector(() => document.querySelector(`axa-footer-small`).shadowRoot.querySelector('.m-footer-small__disclaimer .m-footer-small__list')).innerText;

  await t.expect(disclaimers).contains('Nutzungshinweise  Datenschutz');

  const italian = Selector(() => document.querySelector(`axa-footer-small`).shadowRoot.querySelector('.js-footer-small__link-3'));

  await t.click(italian);

  const italianDisclaimers = await Selector(() => document.querySelector(`axa-footer-small`).shadowRoot.querySelector('.m-footer-small__disclaimer .m-footer-small__list')).innerText;

  await t.expect(italianDisclaimers).contains("Avvertenze per l'utilizzazione  Protezione dei dati");
});
