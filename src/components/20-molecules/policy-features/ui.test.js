...
const $axaPolicyFeaturesHeaderEl = Selector(() => document.querySelector(TAG).shadowRoot, { dependencies: { TAG } }).find('h1');

const $axaPolicyFeaturesArticleEl = Selector(() => document.querySelector(TAG).shadowRoot, { dependencies: { TAG } }).find('article');
...
test('should render title', async t => {
  await t.expect($axaPolicyFeaturesHeaderEl.textContent).eql('A 5 star car insurance with affordable premium services');
});

test('should render default background (dark-indigo)', async t => {
  await t.expect(await $axaPolicyFeaturesArticleEl.getStyleProperty('background-color')).eql(DEFAULT_BACKGROUND_RGB);
});

test('should render title with correct size on screen lg', async t => {
  await t.expect(await $axaPolicyFeaturesHeaderEl.getStyleProperty('font-size')).eql('48px');
}).before(async t => {
...
test('should render title with correct size on all other screen sizes', async t => {
  await t.expect(await $axaPolicyFeaturesHeaderEl.getStyleProperty('font-size')).eql('30px');
}).before(async t => {
  await t.resizeWindow(large1WindowWidth, defaultWindowHeight);
});

fixture('Policy features - attribute variant: not set').page(`${host}/iframe.html?id=components-policy-features--policy-features&knob-variant=&knob-title=A 5 star car insurance with affordable premium services&knob-Show title?=y&knob-title (of item)=Get Discount&knob-icon=email&knob-icon - load svg icon from this url instead:=&knob-description=A 5 star car insurance with affordable premium services`);

test('should set default background-color if variant is not set', async t => {
  await t.expect(await $axaPolicyFeaturesArticleEl.getStyleProperty('background-color')).eql(DEFAULT_BACKGROUND_RGB);
});

fixture('Policy features - attribute variant: not in whitelist').page(`${host}/iframe.html?id=components-policy-features--policy-features&knob-variant=thisStyleIsNotInWhitelist&knob-title=A 5 star car insurance with affordable premium services&knob-Show title?=y&knob-title (of item)=Get Discount&knob-icon=email&knob-icon - load svg icon from this url instead:=&knob-description=A 5 star car insurance with affordable premium services`);
